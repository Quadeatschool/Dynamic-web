...
{
    const randomRecipeIndex = randomNumber(recipes.length);
...
function recipeTemplate(recipe) {
    return `<figure class="recipe">
        ...
        <figcaption class="recipe__content">
...
function searchListObjects(list, query) {
  function searchCallbackObject(item) {
    return (
  ...
  const filtered = list.filter(searchCallbackObject);
  return filtered;
...
document.querySelector('.search-bar').addEventListener('submit', function(event) {
  ...
  const results = searchListObjects(recipes, query);
  document.getElementById('recipe-container').innerHTML = results.length
    ? results.map(recipeTemplate).join('')
